...
> 状态：`0.2.0-preview.3` 实施基线。本文同时记录已实现决策和正式版仍需完成的外部验证。
...
| ADR-009 | 一条长期 `main`、短期任务分支和 CNB 原生三环境晋级         | 已接受     | 真机验收 CNB H5 发布入口             |
...
2. 告知建议名称、最小权限和有效期。
3. 用户粘贴一次，ABCDeploy 立即验证并存入系统密钥库；最小权限为 `repo-code:rw`、`repo-manage:rw`、`repo-cnb-trigger:rw`、`group-resource:rw`。
...
CNB Secret 类型仓库不能 clone，当前支持的公开 API 也不提供文件写入。ABCDeploy 生成带中文说明的测试、生产配置，分别复制到剪贴板并打开准确的 CNB Web 页面；用户完成一次保存后，流水线只引用变量名。界面必须明确这是 CNB 的安全审计步骤，不能宣称全自动。
...
- 用户可在 CNB 部署页面或 ABCDeploy 执行同一个生产发布动作，不依赖 `production` 审批分支。
...
完整流程见 [分支、环境与版本晋级](../deployment-model.md)。
