# DeployDesk V2 用户旅程与信息架构

## 1. 核心交互模型

DeployDesk 有两个明确模式：

- **初始化向导**：为第一次部署服务，不显示后台式侧栏。
- **项目工作台**：为日常部署和多项目管理服务，不重复初始化问题。

不能再让同一组菜单同时承担“操作步骤”和“日常导航”。
...
1. **CNB 账号**：点击授权，浏览器登录并自动返回。
2. **构建仓库**：优先匹配同名已有仓库；没有时建议创建私有仓库。
3. **目标服务器**：填写 IP 或域名，自动尝试可用身份。
4. **域名**：首次 staging 可跳过，正式上线前再处理。
...
按照“从哪里获得”分组，不按照环境变量名排序：

- **需要登录第三方平台获取**：AI API Key、短信凭证、支付凭证。
- **已有服务信息**：数据库地址、对象存储地址。
- **域名**：可跳过 staging，production 前必须完成。

系统自动生成 JWT Secret、部署 Token、数据库名称、容器命名空间和 Redis 前缀。
...
- 将向项目添加 8 个部署文件。
- 将在服务器创建 DeployDesk 目录、网络和 Caddy 配置。
- 不会删除现有容器、目录或数据库。
...
> DeployDesk 已连接 CNB，但当前授权不能创建仓库。
