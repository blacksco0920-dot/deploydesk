...
> 对齐日期：2026-07-21。本文只归纳可复用的技术能力和架构约束，不规定页面文案。面向用户的四节点线路以[产品实施合同](product-contract.md)为唯一权威；本文记录该产品合同背后的真实执行能力。
...
| --- | --- | --- | --- |
| 工作流表现层 | 已直接集成 FlowGram 画布、节点、连线、选择/抓手、缩放、适应视图、历史和吸附；节点坐标按部署线路持久化，进入或切换线路时恢复布局后自动 `fitView`，打开或关闭 Inspector 只调整画布容器尺寸，不重置用户视口；Semi Design 已用于画布通用工具控件；Coze Studio 只作为三栏工作区、资源栏和 Inspector 的结构与交互基线，没有整体导入其业务前端 | 运行时代码 + 自动化测试，本地桌面验收通过 | `apps/desktop/src/components/DeploymentWorkflowCanvas.tsx`、`apps/desktop/src/components/DeploymentPathWorkspace.tsx`、`apps/desktop/src/api.ts`、`apps/desktop/package.json` |
...
| 工作流基础交互 | 新建线路后自动打开距离代码来源最近的未就绪节点；节点卡同时显示 Provider、当前 Connection 名称和文字状态；Inspector 支持 `Escape` 关闭并把焦点还给触发节点；运行记录图标具有可见 Tooltip；线路列表常显重命名与删除入口，删除经二次确认，删除最后一条后保持无线路状态 | 运行时代码 + 自动化测试 | `apps/desktop/src/components/DeploymentPathWorkspace.tsx`、`apps/desktop/src/components/DeploymentWorkflowCanvas.tsx` |
